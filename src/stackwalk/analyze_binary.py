...
def get_clue_table(input):
    ...
    clue_table = [(addr, meta["cfa_offset"], meta["ip_offset"], meta["function_name"], meta["handler_indicator"], meta["in_handler"]) for addr, meta in clue_table.items()]
...
def clean_clue_table(clue_table):
    ...
    return [(addr, cfa_offset, ip_offset, function_name, handler_indicator, in_handler) for addr, cfa_offset, ip_offset, function_name, handler_indicator, in_handler in clue_table if function_name not in irrelevant_functions]
...
def main():
    ...
    with open("clue_table.txt", "w") as file:
        ...
        for addr, cfa_offset, ip_offset, function_name, handler_indicator, in_handler in clue_table:
            file.write(f"{addr:#x} {cfa_offset} {ip_offset} {function_name} {handler_indicator} {in_handler}\n")
