...
uint64_t mod(uint64_t a, uint64_t b, i64 *abort_stub) {
    if (b == 0)
        RAISE(abort_stub, 0, ());
    return a % b;
}
...
__attribute((noinline))
uint64_t divide(uint64_t a, uint64_t b, i64 *abort_stub) {
    if (b == 0)
        RAISE(abort_stub, 0, ());
    return a / b;
}
...
void extendedEuclid(uint64_t a, uint64_t b, uint64_t *x, uint64_t *y, uint64_t *gcd, i64 *abort_stub) {
    if (b == 0) {
        *x = 1;
        *y = 0;
        *gcd = a;
    ...
    *x = y1;
    *y = x1 - divide(a, b, abort_stub) * y1;
}

FAST_SWITCH_DECORATOR
i64 extendedEuclid_handle_body(i64 *env, i64 *abort_stub) {
    ...
    extendedEuclid(a, b, x, y, gcd, abort_stub);
    return 0;
}
