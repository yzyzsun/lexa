...
int hofstadterq(int n, i64 *abort_stub) {
    if(n <= 0) RAISE(abort_stub, 0, ((i64)n));
    else if(n <= 2) return 1;

    return hofstadterq(n - hofstadterq(n - 1, abort_stub), abort_stub) 
            + hofstadterq(n - hofstadterq(n - 2, abort_stub), abort_stub);
}
